...
Wanny 想做的，不只是“能控制几个设备”的智能家居，而是一个真正理解你生活节奏、主动把一切安排妥当的 AI 管家。

想象这样一个下班回家的傍晚：

- 你的车刚从公司出发，Jarvis 结合车辆位置、回家路线和到家时间，判断你已经在回家的路上。
- 它不会只做“提醒”，而是开始联动真实设备: 提前启动电饭锅，让晚饭在你到家前进入刚刚好的节奏。
- 它会顺手查看当前天气和室内外温度，如果降温明显、家里偏冷，就提前打开地暖，让你推门的一刻就是舒适温度。
- 如果车辆油量已经偏低，它还可以结合你当前路线、附近站点分布，以及最近现实世界中油价上涨的消息，主动推荐更合适的加油站，帮你把补能决策前置。

我们希望 Wanny 最终能把“设备控制”“环境感知”“出行状态”“现实资讯”“长期记忆”和“主动决策”真正串起来，让 AI 不只是回答问题，而是参与生活、理解上下文、在合适的时机把事情做到刚刚好。

这也是项目持续演进的方向：接入更多设备、连接更多现实世界信号、打通更多场景，让每一个自动化动作都更像一个贴心的人，而不是一条冰冷规则。
...
- **硬件对接**: Mijia API (`mijiaAPI`)、美的、海信、奔驰
...
- Docker 部署指南见 [docs/DEPLOYMENT.md](./docs/DEPLOYMENT.md)。
- 项目介绍与系统设计总览见 [2026-03-29-wanny-design.md](./docs/superpowers/specs/2026-03-29-wanny-design.md)。
- 主动关怀设计与当前实现状态见 [2026-04-03-proactive-care-design.md](./docs/superpowers/specs/2026-04-03-proactive-care-design.md)。
- roadmap 见 [docs/ROADMAP.zh-CN.md](./docs/ROADMAP.zh-CN.md)。
- 详细专项设计、实施记录与规格说明位于 [`docs/superpowers/specs`](./docs/superpowers/specs)。

## 💡 主动关怀快速上手

1. 先完成至少一个设备平台授权，按需接入 Home Assistant。
2. 打开控制台，进入 `关怀 / Care`。
3. 在 `规则` 页面先添加天气数据源。
4. 查看系统规则，或通过条件构建器新建自定义规则。
5. 点击 `立即巡检` 或刷新天气，生成新的关怀建议。
6. 在关怀中心里对建议执行采纳、拒绝、忽略或直接执行。

当前后端/前端覆盖范围、调度方式和剩余工作，请查看 [2026-04-03-proactive-care-design.md](./docs/superpowers/specs/2026-04-03-proactive-care-design.md)。
...
- `third_party/HisenseHA` 以 Git submodule 形式引入，作为海信接入的参考实现源码。上游协议为 MIT License。
...
- 致谢上游项目 [`manymuch/HisenseHA`](https://github.com/manymuch/HisenseHA) 提供的 Home Assistant 集成与相关实现思路。
