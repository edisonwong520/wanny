...
- **层级表达**：Console 默认通过边框、底色差异和留白建立层级，不依赖灰色阴影；尤其是顶部 tab、主面板和核心卡片，应优先避免使用灰色投影。
...
- **按钮可读性**：所有按钮和可点击操作入口必须优先保证文字清晰可见。对于小尺寸按钮，优先使用“浅底 + 深色文字”的高对比方案，不要默认使用品牌色底加白字；如果某个按钮出现过文字不可见问题，应直接切换为更稳的高对比样式，而不是继续微调阴影或透明度。
