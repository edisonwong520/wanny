...
class WeChatService:
    ...
    async def process_incoming_message(cls, message, bot):
        ...
        if not content:
            logger.debug(f"[WeChat Service] 收到空消息，已忽略。user_id={wechat_user_id}")
            return

        logger.info(f"[WeChat Service] 收到消息: user_id={wechat_user_id}, 内容='{content[:50]}'")

        # 尝试查找关联的系统账户
        account = await cls.get_account_by_wechat_id(wechat_user_id)
        if not account:
            logger.warning(f"[WeChat Service] ⚠️ 收到未分配账户的消息: wechat_user_id={wechat_user_id}。将以受限模式运行。")
        ...
        try:
            ...
            memory_context = ""
            if account:
                memory_context = await MemoryService.get_context_for_chat(account, content)
            ...
            if intent_type in ["CONFIRM", "PERMANENT_ALLOW"]:
                ...
                try:
                    if mission.shell_command:
                        result_msg = await ShellExecutor.execute_yolo(mission.shell_command)
                    else:
