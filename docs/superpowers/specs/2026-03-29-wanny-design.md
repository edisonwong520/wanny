...
- **Device Providers**: 当前已接入米家、美的 IoT 与奔驰 `mbapi2020`。
- **Database**: **MySQL** (核心存储)。
...
/wanny
  /backend           # Django 项目根目录 (wanny_server)
    /apps
      /devices       # 设备管理、统一设备视图与控制执行
      /brain         # LLM Agent、记忆系统
      /comms         # 微信网关 (weclaw) 接入
      /database      # 核心存储与 ORM 模型
      /memory        # 记忆系统实现
      /providers     # 第三方服务接入 (如 MiCloud / Midea / Mercedes-Benz)
...
- 奔驰 `mbapi2020` 接入当前以 git submodule 形式放在 [`third_party/mbapi2020`](/Users/edison/code/python/wanny/third_party/mbapi2020)，上游项目来源与致谢为 [ReneNulschDE/mbapi2020](https://github.com/ReneNulschDE/mbapi2020)。
- 当前 Wanny 业务层没有直接复用其 Home Assistant 登录流程，而是在 [`backend/apps/providers/clients/mbapi2020/client.py`](/Users/edison/code/python/wanny/backend/apps/providers/clients/mbapi2020/client.py) 中按本项目约束做了 token 直连封装与设备控制适配。
