...
当前前端实现已经根据真实交互反馈做了第一轮收敛，后续改动应优先遵守下面这些约定：

- 顶部只保留 4 个一级 tab：`总览`、`任务`、`设备`、`管理`
- 顶部右侧只保留 `当前模式` 和中英切换，不再展示品牌头像、运行时说明或开发者状态文案
- `管理` 合并原先的 `Memory Atlas` 与 `Guard Rails`
- 每个 tab 进入后直接展示内容区，不再重复放一块“当前页面介绍”式的顶部说明面板
- `总览` 只保留用户关心的关键卡片和最近动态，不展示复盘时间、向量库实现、轮询细节等内部信息
- 概览卡片避免再补一行和主值重复的小字说明
- 任何“打开管理”“去任务页”这一类明确操作入口，都应使用按钮样式，而不是纯文本链接
